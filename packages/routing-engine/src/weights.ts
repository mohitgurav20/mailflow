...
export interface ClassObjectiveWeights {
  timeWeight: number;       // Importance of total transit duration
  costWeight: number;       // Importance of financial cost
  reliabilityWeight: number;// Importance of historical on-time probability
  carbonWeight: number;     // Importance of carbon footprint
}
 ...
 */
export const CLASS_WEIGHTS: Record<MailClass, ClassObjectiveWeights> = {
  SPEED_POST: {
    timeWeight: 0.65,
...
export function calculateEdgeCost(params: {
  ...
  return (
    weights.timeWeight * normDuration +
 ...
 */
export function calculateRouteConfidence(reliabilities: number[]): number {
