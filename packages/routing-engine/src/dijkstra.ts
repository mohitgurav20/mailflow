import { RouteOption, RouteLeg, RouteComputeRequest, Hub } from '@mailflow/shared-types';
import { MultimodalNetworkGraph, TimeExpandedLegInstance } from './graph.js';
...
export class MultimodalDijkstraRouter {
  constructor(private graph: MultimodalNetworkGraph) {}

  /**
   * Computes top diverse routes between origin and destination using time-expanded Dijkstra search.
   */
  public findRoutes(request: RouteComputeRequest): RouteOption[] {
    ...
    const fastestRoute = this.searchSingleObjective({
      ...
      excludeRouteLegs: optimalRoute ? optimalRoute.legs.map((l: RouteLeg) => l.legId) : []
    });
    ...
    if (economicalRoute && (!optimalRoute || economicalRoute.totalCost < optimalRoute.totalCost)) {
    ...
    const seenLegSignatures = new Set<string>();

    for (const opt of candidates) {
      const sig = opt.legs.map((l: RouteLeg) => l.legId).join('|');
    ...
    if (baseline) {
      uniqueOptions.forEach((opt, idx) => {
        ...
        opt.tradeoffs = {
          ...
          costVsOptimalINR: Math.round((opt.totalCost - baseline.totalCost) * 100) / 100,
  ...
  private searchSingleObjective(params: {
    request: RouteComputeRequest;
    startDateTime: Date;
    weightKg: number;
    mailClass: RouteComputeRequest['mailClass'];
    maxTransfers: number;
    objectiveOverride: 'BALANCED' | 'FASTEST' | 'ECONOMICAL';
    excludeRouteLegs?: string[];
  }): RouteOption | null {
    ...
    const minCostToHub = new Map<string, number>();
    minCostToHub.set(request.originHubId, 0);

    let bestSolution: DijkstraState | null = null;
    ...
    while (queue.length > 0) {
      ...
      for (const leg of outboundLegs) {
        // Avoid cycles
        if (state.visitedHubs.has(leg.destHubId)) continue;

        // Avoid excluded legs (for K-shortest diversification)
        if (excludeRouteLegs.includes(leg.id) && state.legs.length === 0) continue;

        // Check user avoid lists
        if (request.avoidHubs && request.avoidHubs.includes(leg.destHubId)) continue;
        if (request.avoidLegs && request.avoidLegs.includes(leg.id)) continue;
        if (request.preferredModes && !request.preferredModes.includes(leg.mode)) continue;

        // Check Embargo
        if (this.graph.isEmbargoed(leg.destHubId, leg)) continue;
